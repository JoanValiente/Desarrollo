...
void j1Map::PropagateBFS()
{
	...
	//visited red    frontier green
	
		iPoint current_data;
		if (frontier.start != NULL) {
			//current_data = frontier.start->data;
			frontier.Pop(current_data);
			if (visited.find(iPoint((current_data.x - 1), (current_data.y))) == -1) {
				if (IsWalkable(current_data.x - 1, current_data.y)) {
					frontier.Push(iPoint((current_data.x - 1), (current_data.y)));
				}
			}
			...
			if (visited.find(iPoint((current_data.x), (current_data.y + 1))) == -1) {
				if (IsWalkable(current_data.x, current_data.y +1)) {
					frontier.Push(iPoint((current_data.x), (current_data.y + 1)));
				}
			}
			if (visited.find(iPoint((current_data.x), (current_data.y - 1))) == -1) {
				if (IsWalkable(current_data.x, current_data.y - 1)) {
	...
	// to the frontier queue and visited list
		if (visited.find(iPoint(current_data)) == -1) {
