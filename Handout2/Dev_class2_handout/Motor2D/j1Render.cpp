...
void j1Render::RealSave(pugi::xml_node& savedoc) const {
	if (savedoc.child("camera") == NULL) {
		savedoc.append_child("camera").append_attribute("x") = camera.x;
		savedoc.child("camera").append_attribute("y") = camera.y;
	}
	else {
		savedoc.child("camera").attribute("x").set_value(camera.x);
		savedoc.child("camera").attribute("y").set_value(camera.y);
	}
}

void j1Render::RealLoad(pugi::xml_node& savedoc) {
